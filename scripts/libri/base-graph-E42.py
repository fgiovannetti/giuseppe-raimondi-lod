...
with open('../../input/libri.csv', mode='r') as csv_file:
	...
	for row in csv_reader:
		...
		d.add((URIRef(record + 'inventory-number'), RDFS.label, Literal(inventario), graph_base))
		...
		d.add((URIRef(record + 'shelfmark'), RDFS.label, Literal(sezione + ' ' + collocazione + ' ' + specificazione), graph_base))
		...
		d.add((URIRef(series + '/shelfmark'), RDFS.label, Literal(sezione + ' ' + collocazione), graph_base))
...
# TriG
d.serialize(destination="../../dataset/trig/libri_base-graph-E42.trig", format='trig')

# N-Quads
d.serialize(destination="../../dataset/nquads/libri_base-graph-E42.nq", format='nquads')