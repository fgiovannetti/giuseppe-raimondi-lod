...
""" Base graph :
Quaderni, E89 Propositional Object
"""
...
with open('../../input/quaderni.csv', mode='r') as csv_file:
	...
	for row in csv_reader:
		...
		d.add((URIRef(file), ficlitdlo.formsConceptuallyPartOf, URIRef(subseries), graph_base)) # Quaderni manoscritti 1954-1976
		...
		d.add((URIRef(subfonds), RDF.type, URIRef('http://erlangen-crm.org/current/E89_Propositional_Object'), graph_base))
		d.add((URIRef(subfonds), RDFS.label, Literal(subfonds_label_it, lang='it'), graph_base))
		d.add((URIRef(subfonds), RDFS.label, Literal(subfonds_label_en, lang='en'), graph_base))
		d.add((URIRef(subfonds), ecrm.P2_has_type, ficlitdlo.fonds, graph_base))
		d.add((URIRef(subfonds), ficlitdlo.formsConceptuallyPartOf, URIRef(fonds), graph_base)) # Fondo Giuseppe Raimondi (Archivio e Biblioteca)
		d.add((URIRef(subfonds), ficlitdlo.isConceptuallyComposedOf, URIRef(series), graph_base))

		# Library

		d.add((URIRef(subfonds_b), RDF.type, URIRef('http://erlangen-crm.org/current/E89_Propositional_Object'), graph_base))
		d.add((URIRef(subfonds_b), RDFS.label, Literal(subfonds_b_label_it, lang='it'), graph_base))
		d.add((URIRef(subfonds_b), RDFS.label, Literal(subfonds_b_label_en, lang='en'), graph_base))
		d.add((URIRef(subfonds_b), ecrm.P2_has_type, ficlitdlo.fonds, graph_base))
		d.add((URIRef(subfonds_b), ficlitdlo.formsConceptuallyPartOf, URIRef(fonds), graph_base)) # Fondo Giuseppe Raimondi (Archivio e Biblioteca)

		# Fonds description
	
		d.add((URIRef(fonds), RDF.type, URIRef('http://erlangen-crm.org/current/E89_Propositional_Object'), graph_base))
		d.add((URIRef(fonds), RDFS.label, Literal(fonds_label_it, lang='it'), graph_base))
		d.add((URIRef(fonds), RDFS.label, Literal(fonds_label_en, lang='en'), graph_base))
		d.add((URIRef(fonds), ecrm.P2_has_type, ficlitdlo.fonds, graph_base))
		d.add((URIRef(fonds), ficlitdlo.isConceptuallyComposedOf, URIRef(subfonds), graph_base))
		d.add((URIRef(fonds), ficlitdlo.isConceptuallyComposedOf, URIRef(subfonds_b), graph_base))



# TriG
d.serialize(destination="../../dataset/trig/quaderni_base-graph-E89.trig", format='trig')

# N-Quads
d.serialize(destination="../../dataset/nquads/quaderni_base-graph-E89.nq", format='nquads')