...
with open('../../input/corrispondenza.csv', mode='r') as csv_file:
	...
	for row in csv_reader:
		...
		d.add((URIRef(file + '/inventory-number'), RDF.type, ecrm.E42_Identifier, graph_base))
		d.add((URIRef(file + '/inventory-number'), RDFS.label, Literal(inventario[0]), graph_base))
		d.add((URIRef(file + '/inventory-number'), ecrm.P2_has_type, URIRef('https://w3id.org/ficlitdl/ontology/inventory-number'), graph_base))
		...
		d.add((URIRef(series + '/shelfmark'), RDFS.label, Literal(sezione + ' Corrispondenza'), graph_base))
...
# TriG
d.serialize(destination="../../dataset/trig/corrispondenza_base-graph-E42.trig", format='trig')

# N-Quads
d.serialize(destination="../../dataset/nquads/corrispondenza_base-graph-E42.nq", format='nquads')