...
with open('../../input/album.csv', mode='r') as csv_file:
	...
	for row in csv_reader:
		...
		d.add((URIRef(file_text + '/creation'), RDF.type, efrbroo.F28_Expression_Creation, graph_base))
		d.add((URIRef(file_text + '/creation'), efrbroo.R17_created, URIRef(file_text), graph_base))
		d.add((URIRef(file_text + '/creation'), efrbroo.R18_created, URIRef(file_object), graph_base))
		#d.add((URIRef(file_text + '/creation'), URIRef('http://erlangen-crm.org/current/P4_has_time-span'), URIRef(base_uri + 'time-span/' + timespan[0]), graph_base))
		d.add((URIRef(file_text + '/creation'), URIRef('http://erlangen-crm.org/current/P4_has_time-span'), URIRef(base_uri + 'time-span/1977', graph_base)))
		d.add((URIRef(file_text + '/creation'), ecrm.P14_carried_out_by, URIRef(person + 'giuseppe-raimondi'), graph_base))
...
# TriG
d.serialize(destination="../../dataset/trig/album_base-graph-F28.trig", format='trig')

# N-Quads
d.serialize(destination="../../dataset/nquads/album_base-graph-F28.nq", format='nquads')