...
with open('../../input/album.csv', mode='r') as csv_file:
	...
	for row in csv_reader:
		...
		d.add((URIRef(file_text + '/author'), RDF.type, pro.RoleInTime, graph_base))
		d.add((URIRef(file_text  + '/author'), pro.withRole, pro.author, graph_base))
		d.add((URIRef(file_text  + '/author'), RDFS.label, Literal("Giuseppe Raimondi, autore dell'album " + '"' + label[0] + '"', lang='it'), graph_base))
...
# TriG
d.serialize(destination="../../dataset/trig/album_base-graph-RIT.trig", format='trig')

# N-Quads
d.serialize(destination="../../dataset/nquads/album_base-graph-RIT.nq", format='nquads')