...
""" Base graph :
Album, F22 Self-Contained Expression
"""
...
with open('../../input/album.csv', mode='r') as csv_file:
	...
	for row in csv_reader:
		...
		d.add((URIRef(file_expression), RDF.type, URIRef('http://erlangen-crm.org/efrbroo/F22_Self-Contained_Expression'), graph_base))
		d.add((URIRef(file_expression), ecrm.P128i_is_carried_by, URIRef(file_object), graph_base))
		d.add((URIRef(file_expression), efrbroo.R17i_was_created_by, URIRef(file_expression + '/creation'), graph_base))
		d.add((URIRef(file_expression), ecrm.P104_is_subject_to, URIRef('https://w3id.org/ficlitdl/right/all-rights-reserved'), graph_base))
		d.add((URIRef(file_expression), ecrm.P102_has_title, URIRef(file_expression + '/title'), graph_base))
...
# TriG
d.serialize(destination="../../dataset/trig/album_base-graph-F22.trig", format='trig')

# N-Quads
d.serialize(destination="../../dataset/nquads/album_base-graph-F22.nq", format='nquads')