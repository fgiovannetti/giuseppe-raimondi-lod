# coding: utf-8

# address

import csv
import re
import rdflib_jsonld
from rdflib import Graph, Literal, BNode, Namespace, URIRef
from rdflib.namespace import RDF, RDFS, XSD, DCTERMS, OWL, SKOS

ecrm = Namespace("http://erlangen-crm.org/current/")

g = Graph()

g.bind("ecrm", ecrm)
g.bind("dcterms", DCTERMS)
g.bind("owl", OWL)

base_uri = 'https://w3id.org/ficlitdl/'

g.add((URIRef(base_uri + 'address/' + 'via-zamboni-32-40126-Bologna-BO'), RDF.type, ecrm.E45_Address))
g.add((URIRef(base_uri + 'address/' + 'via-zamboni-32-40126-Bologna-BO'), RDFS.label, Literal('Via Zamboni 32, 40126, Bologna (BO), Italia' , lang='it')))
g.add((URIRef(base_uri + 'address/' + 'via-zamboni-32-40126-Bologna-BO'), RDFS.label, Literal('Via Zamboni 32, 40126, Bologna (BO), Italy' , lang='en')))

# RDF/XML
g.serialize(destination="../output/rdf/E45.rdf", format='xml')

# Turtle
g.serialize(destination="../output/ttl/E45.ttl", format='ttl')

# JSON-LD
g.serialize(destination="../output/jsonld/E45.jsonld", format='json-ld')