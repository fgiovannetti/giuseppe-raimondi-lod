...
with open('../input/quaderni.csv', mode='r') as csv_file:
	...
	for row in csv_reader:
		...
		g.add((rec_object, RDF.type, URIRef('http://erlangen-crm.org/current/E22_Human-Made_Object')))
		...
		g.add((rec_object, ecrm.P1_is_identified_by, URIRef(record + 'inventory-number')))
		g.add((rec_object, ecrm.P1_is_identified_by, URIRef(record + 'shelfmark')))

		# Document type (notebooks are regarded both as items and files)
		g.add((rec_object, ecrm.P2_has_type, ficlitdlo.notebook))
		g.add((rec_object, ecrm.P2_has_type, ficlitdlo.file))
		
		# Descrizione isbd
		g.add((rec_object, ecrm.P3_has_note, Literal(descrizione_isbd.replace('*', '') , lang='it')))
		...
		if '449' in inventario[0]: 
			...
			g.add((rec_object, ecrm.P43_has_dimension, URIRef(base_uri + 'height/' + height[0] + 'cm')))
			g.add((rec_object, ecrm.P43_has_dimension, URIRef(base_uri + 'extent/' + extent_pages[0] + 'p'))) 
			g.add((rec_object, ecrm.P43_has_dimension, URIRef(base_uri + 'extent/' + extent_leaves[0] + 'c')))

		# Fa parte di
		if collocazione == 'QUADERNI.1':
			g.add((rec_object, ecrm.P46i_forms_part_of, URIRef(file + '/object')))
			g.add((rec_object, ficlitdlo.formsConceptuallyPartOf, URIRef(file)))
		else:
			g.add((rec_object, ecrm.P46i_forms_part_of, URIRef(subseries + '/object')))
			g.add((rec_object, ficlitdlo.formsConceptuallyPartOf, URIRef(subseries)))
		...
		if collocazione == 'QUADERNI.1':
			if inventario[0] == 'RDq 84':
				...
				if str(seqn_prev) in invn_by_seqn[specificazione]:
					g.add((rec_object, seq.follows, URIRef(base_uri + 'notebook/rdq' + invn_by_seqn[specificazione][str(seqn_prev)] + '/object')))
				if str(seqn_next) in invn_by_seqn[specificazione]:
					g.add((rec_object, seq.precedes, URIRef(base_uri + 'notebook/rdq' + invn_by_seqn[specificazione][str(seqn_next)] + '/object')))
		...
		g.add((rec_object, ecrm.P50_has_current_keeper, URIRef('https://w3id.org/ficlitdl/org/unibo')))
		g.add((rec_object, ecrm.P52_has_current_owner, URIRef('https://w3id.org/ficlitdl/org/ibc')))
		g.add((rec_object, ecrm.P55_has_current_location, URIRef('https://w3id.org/ficlitdl/place/biblioteca-ezio-raimondi')))
		g.add((rec_object, ecrm.P104_is_subject_to, URIRef('https://w3id.org/ficlitdl/right/' + 'all-rights-reserved')))
		# Link to overall expression
		g.add((rec_object, ecrm.P128_carries, rec_expression))
...
# RDF/XML
g.serialize(destination="../output/rdf/quaderni-E22.rdf", format='xml')

# Turtle
g.serialize(destination="../output/ttl/quaderni-E22.ttl", format='ttl')

# JSON-LD
g.serialize(destination="../output/jsonld/quaderni-E22.jsonld", format='json-ld')